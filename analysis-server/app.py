# app.py (Flask 분석 전용 버전 - GPT 음악추천 통합 + 주간리포트 완성)
...
def _to_json_safe(v):
    ...
    if _decimal and isinstance(v, _decimal.Decimal):
        try: return float(v)
        except: return str(v)
    if isinstance(v, dict): return {str(k): _to_json_safe(val) for k, val in v.items()}
    if isinstance(v, (list, tuple, set)): return [_to_json_safe(x) for x in v]
    return str(v)
...
def analyze_file():
    ...
    if not orig: return _json_err("empty filename", 400)
    ...
    else: return _json_err(f"unsupported file type: {ext}", 415)
    ...
    try:
        ...
        if fb:
            result["feedback"] = fb
            logger.info(f"[GPT] Feedback 생성 완료")

        # === GPT 음악 추천 ===
        try:
            top_emotion = max(fused_dist, key=fused_dist.get) if fused_dist else "neutral"
            songs = gpt.make_music_recs(top_emotion, count=5)
            if songs:
                result["music"] = songs
                logger.info(f"[GPT] Music 추천 {len(songs)}곡 생성 완료 ({top_emotion})")
        except Exception as e:
            logger.warning(f"Music recommendation failed: {e}")

        logger.info("[DB Disabled] 결과 저장 안 함 (전달만)")
...
def report_weekly():
    try:
        ...
        for r in rows:
            ...
            if isinstance(emo_raw, str):
                try:
                    ...
                    if isinstance(emo, str):  # 🔥 nested JSON 처리
                        emo = json.loads(emo)
                except Exception:
                    emo = {}
            ...
            if not dist and all(k in emo for k in keys):
                dist = emo
        ...
        fb = gpt.make_feedback(f"{start}~{end}", avg, "")
